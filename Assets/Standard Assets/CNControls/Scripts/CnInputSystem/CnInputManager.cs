...
namespace CnControls
{
    ...
    public class CnInputManager
    {
		...
		// edited by abdul rafay 
		public static void ReleaseButton(string name)
		{
			if (ButtonExists (name)) 
			{
				for (int i = 0; i < Instance._virtualButtonsDictionary[name].Count; i++) 
				{
					if(Instance._virtualButtonsDictionary[name][i].Name == name)
						Instance._virtualButtonsDictionary[name] [i].Release();
				}
			}
		}
        /// <summary>
